...
## actual solution (2026-01-28)
There used to be a section here, but it was wrong.

tldr: Steam on NixOS runs inside of a container, so you cant really just use protontricks to launch teamspeak, it does work for the installer though.

What I now use is `steamtinkerlaunch`, install it, set it as the compatibility tool in steam, hit play, click the main menu button quickly before it disappears, go to game menu, add custom command, select the teamspeak executable, click use custom command, click inject custom command, scroll down and set your proton version... and now magically arma and teamspeak both launch at the same time, wonderful, only took a year for me to find a reasonable solution.
